...
names(df2)[1] <- 'DATE_REPORT'
names(df2)[2] <- 'STATE'
names(df2)[3] <- 'FIPS'
names(df2)[4] <- 'CASES'
names(df2)[5] <- 'DEATH'

class(df2$Date_report)
str(df2)
df2$DATE_REPORT = as.Date(df2$DATE_REPORT)
df2$WEEK_DAY=weekdays(df2$DATE_REPORT)
...
a=quantile(df2$DEATH)[2]
b=quantile(df2$DEATH)[4]

df2$weekend <- case_when(
  df2$DEATH <a ~ "low_death",
  df2$DEATH <b ~ "mid_death",
  TRUE~"high_Death")
...
```{r}
#trying to figure out which of the survivors' classes is most expensive
df3=read_csv("C:\\Users\\Farida Yesmin\\Desktop\\M421 RStats\\aroshenoor.github.io\\titanic.csv")
df3 %>% 
  filter(Survived==1) %>%
  group_by(Pclass) %>% 
  summarise(Fare = max(Fare)) %>%
  arrange(desc(Fare))
...
```{r}
df3 %>% mutate(deaths = ifelse(Survived==0, "Survived", "Death") )
```

- summarise

```{r}
df3 %>% summarise(df3, "SibSp", "Parch", .group = NULL)
```
